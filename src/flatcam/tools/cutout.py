...
from flatcam.core.tool_base import AppTool
...
class CutOut(AppTool):
    ...
    def run(self, toggle=True):
        ...
        if toggle:
            ...
            if self.app.ui.splitter.sizes()[0] == 0:
                ...
                try:
                    if (
                        self.app.ui.tool_scroll_area.widget().objectName()
                        == self.toolName
                    ):
                        # if tab is populated with the tool but it does not have the focus, focus on it
                        if (
                            not self.app.ui.notebook.currentWidget()
                            is self.app.ui.tool_tab
                        ):
                            # focus on Tool Tab
        ...
        self.default_data.update(
            {
                ...
                "tools_cutout_depthperpass": float(
                    self.app.defaults["tools_cutout_depthperpass"]
                ),
                "tools_cutout_mdepth": self.app.defaults["tools_cutout_mdepth"],
                "tools_cutout_gapsize": float(
                    self.app.defaults["tools_cutout_gapsize"]
                ),
                "tools_cutout_gaps_ff": self.app.defaults["tools_cutout_gaps_ff"],
                "tools_cutout_convexshape": self.app.defaults[
                ...
                "tools_cutout_gap_depth": float(
                    self.app.defaults["tools_cutout_gap_depth"]
                ),
                "tools_cutout_mb_dia": float(self.app.defaults["tools_cutout_mb_dia"]),
                "tools_cutout_mb_spacing": float(
                    self.app.defaults["tools_cutout_mb_spacing"]
                ),
            }
        )
    ...
    def update_ui(self, tool_dict):
        ...
        self.ui.mpass_cb.set_value(tool_dict["tools_cutout_mdepth"])
    ...
    def on_tool_add(self, custom_dia=None):
        ...
        if tool_found == 0:
            self.app.inform.emit(
                "[WARNING_NOTCL] %s"
                % _("Tool not in Tools Database. Adding a default tool.")
            )
        ...
        new_tools_dict["tools_cutout_depthperpass"] = deepcopy(
            new_tools_dict["depthperpass"]
        )
    ...
    def on_cutout_tool_add_from_db_executed(self, tool):
        ...
        if tool["data"]["tool_target"] not in [0, 6]:  # [General, Cutout Tool]
            ...
            self.app.inform.emit(
                "[ERROR_NOTCL] %s"
                % _("Selected tool can't be used here. Pick another.")
            )
            return
        ...
        tool_from_db["data"]["tools_cutout_mdepth"] = deepcopy(
            tool_from_db["data"]["multidepth"]
        )
    ...
    def on_freeform_cutout(self):
        ...
        try:
            ...
            self.app.inform.emit(
                "[ERROR_NOTCL] %s: %s" % (_("Could not retrieve object"), name)
            )
        ...
        with self.app.proc_container.new("Generating Cutout ..."):
            ...
            has_mouse_bites = (
                True if self.ui.gaptype_radio.get_value() == "mb" else False
            )
            ...
            def job_thread(app_obj):
                ...
                if cutout_obj.kind == "gerber":
                    if isinstance(cutout_obj.solid_geometry, list):
                        cutout_obj.solid_geometry = MultiPolygon(
                            cutout_obj.solid_geometry
                        )
                    try:
                        if convex_box:
                        ...
                        log.debug(
                            "CutOut.on_freeform_cutout().geo_init() --> %s" % str(err)
                        )
                        object_geo = cutout_obj.solid_geometry
                else:
                ...
                if kind == "single":
                    ...
                    for geom_struct in object_geo:
                        if cutout_obj.kind == "gerber":
                            if margin >= 0:
                                geom_struct = (
                                    geom_struct.buffer(margin + abs(dia / 2))
                                ).exterior
                            else:
                                geom_struct_buff = geom_struct.buffer(
                                    -margin + abs(dia / 2)
                                )
                ...
                if has_mouse_bites is True:
                    ...
                    if kind == "single":
                        ...
                        for mb_geom_struct in mb_object_geo:
                            if cutout_obj.kind == "gerber":
                                if margin >= 0:
                                    mb_geom_struct = mb_geom_struct.buffer(
                                        margin + mb_buff_val
                                    )
                            ...
                            __, mb_r_geo = cutout_handler(
                                geom=mb_geom_struct, gapsize=gapsize
                            )
                ...
                def geo_init(geo_obj, app_object):
                    ...
                    geo_obj.tools[1]["data"]["multidepth"] = (
                        self.ui.mpass_cb.get_value()
                    )
                    geo_obj.tools[1]["data"]["depthperpass"] = (
                    ...
                    if not gaps_solid_geo:
                        ...
                        geo_obj.tools[9999]["data"]["cutz"] = (
                            self.ui.thin_depth_entry.get_value()
                        )
                        geo_obj.tools[9999]["data"]["multidepth"] = (
                            self.ui.mpass_cb.get_value()
                        )
                        geo_obj.tools[9999]["data"]["depthperpass"] = (
                            self.ui.maxdepth_entry.get_value()
                        )
                ...
                def excellon_init(exc_obj, app_o):
                    ...
                    tools = {
                        1: {"tooldia": mb_dia, "drills": holes, "solid_geometry": []}
                    }
                ...
                try:
                    if self.ui.gaptype_radio.get_value() == "mb":
                        ret = app_obj.app_obj.new_object(
                            "excellon", outname_exc, excellon_init
                        )
                        if ret == "fail":
                            app_obj.inform.emit(
                                "[ERROR_NOTCL] %s" % _("Mouse bites failed.")
                            )
                    ...
                    app_obj.inform.emit(
                        "[success] %s" % _("Any-form Cutout operation finished.")
                    )
    ...
    def on_rectangular_cutout(self):
        ...
        if cutout_obj is None:
            self.app.inform.emit(
                "[ERROR_NOTCL] %s: %s" % (_("Object not found"), str(name))
            )
        ...
        with self.app.proc_container.new("Generating Cutout ..."):
            ...
            def job_thread(app_obj):
                ...
                if kind == "single":
                    ...
                    solid_geo = cutout_rect_handler(
                        geo, gapsize, xmin, ymin, xmax, ymax
                    )
                    ...
                    if cutout_obj.kind == "geometry":
                        ...
                        for geom_struct in object_geo:
                            ...
                            if gap_type == "bt" and thin_entry != 0:
                                try:
                                    gaps_solid_geo += self.subtract_geo(
                                        geom_struct, c_geo
                                    )
                                except TypeError:
                                    gaps_solid_geo.append(
                                        self.subtract_geo(geom_struct, c_geo)
                                    )
                ...
                solid_geo = linemerge(CutOut.flatten(solid_geo))
                ...
                if has_mouse_bites is True:
                    ...
                    if kind == "single":
                        ...
                        mb_solid_geo = cutout_rect_handler(
                            mb_geo, gapsize, xmin, ymin, xmax, ymax
                        )
                        ...
                        if cutout_obj.kind == "geometry":
                            ...
                            for mb_geom_struct in mb_object_geo:
                                ...
                                try:
                                    mouse_bites_geo += self.subtract_geo(
                                        mb_geom_struct, c_geo
                                    )
                                except TypeError:
                                    mouse_bites_geo.append(
                                        self.subtract_geo(mb_geom_struct, c_geo)
                                    )
                        elif cutout_obj.kind == "gerber" and margin >= 0:
                            try:
                                __ = iter(mb_object_geo)
                ...
                def excellon_init(exc_obj, app_o):
                    ...
                    # calculate the bounds
                    e_xmin, e_ymin, e_xmax, e_ymax = CutOut.recursive_bounds(
                        exc_obj.solid_geometry
                    )
    ...
    def on_manual_gap_click(self):
        ...
        self.manual_solid_geo = deepcopy(
            self.flatten(self.man_cutout_obj.solid_geometry)
        )
    ...
    def on_manual_cutout(self, click_pos):
        ...
        new_solid_geometry = CutOut.subtract_geo(
            self.man_cutout_obj.solid_geometry, cut_poly
        )
        new_solid_geometry = linemerge(CutOut.flatten(new_solid_geometry))
        ...
        try:
            ...
            self.man_cutout_obj.tools[1]["data"]["cutz"] = (
                self.ui.cutz_entry.get_value()
            )
            self.man_cutout_obj.tools[1]["data"]["multidepth"] = (
                self.ui.mpass_cb.get_value()
            )
            self.man_cutout_obj.tools[1]["data"]["depthperpass"] = (
                self.ui.maxdepth_entry.get_value()
            )
        except KeyError:
            self.app.inform.emit(
                "[ERROR_NOTCL] %s" % _("No tool in the Geometry object.")
            )
            return
        ...
        if gaps_solid_geo:
            if 9999 not in self.man_cutout_obj.tools:
                ...
                self.man_cutout_obj.tools[9999]["data"]["cutz"] = (
                    self.ui.thin_depth_entry.get_value()
                )
                self.man_cutout_obj.tools[9999]["data"]["multidepth"] = (
                    self.ui.mpass_cb.get_value()
                )
                self.man_cutout_obj.tools[9999]["data"]["depthperpass"] = (
                    self.ui.maxdepth_entry.get_value()
                )
        ...
        def geo_init(geo_obj, app_obj):
            ...
            if convex_box:
                ...
                if (
                    ...
                    geo_obj.solid_geometry = geo_union.buffer(
                        margin + abs(dia / 2)
                    ).exterior
    ...
    def subtract_poly_from_geo(solid_geo, pts):
        ...
        for target in flat_geometry:
            if isinstance(target, (LineString, LinearRing, MultiLineString)):
...
class CutoutUI:
    ...
    def __init__(self, layout, app):
        ...
        self.obj_combo.setRootModelIndex(
            self.app.collection.index(0, 0, QtCore.QModelIndex())
        )
        ...
        self.add_newtool_button.setIcon(
        ...
        # Title2
        title_param_label = QtWidgets.QLabel(
            "<b>%s %s</b>:" % (_("Automatic"), _("Bridge Gaps"))
        )
        ...
        # MANUAL BRIDGE GAPS
        title_manual_label = QtWidgets.QLabel(
            "<b>%s %s</b>:" % (_("Manual"), _("Bridge Gaps"))
        )
        ...
        self.man_gaps_creation_btn.setIcon(
            QtGui.QIcon(self.app.resource_location + "/gaps32.png")
        )
        ...
        self.reset_button.setIcon(
            QtGui.QIcon(self.app.resource_location + "/reset32.png")
        )
    ...
    def confirmation_message(self, accepted, minval, maxval):
        if accepted is False:
            self.app.inform[str, bool].emit(
                "[WARNING_NOTCL] %s: [%.*f, %.*f]"
                % (
                    _("Edited value is out of range"),
                    self.decimals,
                    minval,
                    self.decimals,
                    maxval,
